import { generateWhatsAppStyles } from "@/lib/textStyles";
import { StyleResultItem } from "./StyleResultItem";

type WhatsAppStyleListProps = {
  input: string;
};

export const WhatsAppStyleList = ({ input }: WhatsAppStyleListProps) => {
  const defaultText = "WhatsApp Yazı Stili";
  const styles = generateWhatsAppStyles(input || defaultText);
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1E293B] pb-4">
        <div>
          <div className="text-lg font-bold text-gray-100 font-display">
            WhatsApp Yazı Stilleri
          </div>
          <p className="text-sm text-gray-400 mt-1">
            WhatsApp mesajları ve durumlarınız için özel yazı tipleri oluşturun.
          </p>
        </div>
      </div>
