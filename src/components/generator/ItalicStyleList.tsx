import { generateItalicStyles } from "@/lib/textStyles";
import { StyleResultItem } from "./StyleResultItem";

type ItalicStyleListProps = {
  input: string;
};

export const ItalicStyleList = ({ input }: ItalicStyleListProps) => {
  const defaultText = "İtalik Eğik Yazı";
  const styles = generateItalicStyles(input || defaultText);
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1E293B] pb-4">
        <div>
          <div className="text-lg font-bold text-gray-100 font-display">
            İtalik Eğik Yazı Stilleri
          </div>
          <p className="text-sm text-gray-400 mt-1">
            Metninizi farklı italik eğik unicode karakterlerine dönüştürün.
          </p>
        </div>
      </div>
