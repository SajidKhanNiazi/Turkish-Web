import { generateAestheticStyles } from "@/lib/textStyles";
import { StyleResultItem } from "./StyleResultItem";

type AestheticStyleListProps = {
  input: string;
};

export const AestheticStyleList = ({ input }: AestheticStyleListProps) => {
  const defaultText = "Dekoratif Estetik Yazı Tipleri";
  const styles = generateAestheticStyles(input || defaultText);
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1E293B] pb-4">
        <div>
          <div className="text-lg font-bold text-gray-100 font-display">
            Dekoratif Estetik Yazı Tipleri
          </div>
          <p className="text-sm text-gray-400 mt-1">
            Metninizi yaratıcı, estetik ve süslü şekilli yazılara dönüştürün.
          </p>
        </div>
      </div>
