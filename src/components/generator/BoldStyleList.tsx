import { generateBoldStyles } from "@/lib/textStyles";
import { StyleResultItem } from "./StyleResultItem";

type BoldStyleListProps = {
  input: string;
};

export const BoldStyleList = ({ input }: BoldStyleListProps) => {
  const defaultText = "Kalın Yazı Stili";
  const styles = generateBoldStyles(input || defaultText);
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1E293B] pb-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Metninizi farklı kalın unicode karakterlerine dönüştürün.
          </p>
        </div>
      </div>
