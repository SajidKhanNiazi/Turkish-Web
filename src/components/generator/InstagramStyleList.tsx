import { generateInstagramStyles } from "@/lib/textStyles";
import { StyleResultItem } from "./StyleResultItem";

type InstagramStyleListProps = {
  input: string;
};

export const InstagramStyleList = ({ input }: InstagramStyleListProps) => {
  const defaultText = "Instagram Yazı Stili";
  const styles = generateInstagramStyles(input || defaultText);
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-[#1E293B] pb-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Instagram biyografiniz için en şık yazı tiplerini anında oluşturun.
          </p>
        </div>
      </div>
