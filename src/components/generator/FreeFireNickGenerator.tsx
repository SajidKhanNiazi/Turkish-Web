...
import {
  ...
  FF_READY_NICKS,
  FF_INVISIBLE_CHAR,
  type FreeFireNickItem,
} from "@/lib/freeFireNickStyles";
import { Copy, Check, Zap, Flame, Ghost, Sparkles, ChevronDown } from "lucide-react";
import Link from "next/link";
import { clsx } from "clsx";

const LS_KEY = "ff-nick-input-v1";
...
const NickCard = ({
  ...
  item: FreeFireNickItem;
  index: number;
  onCopied: () => void;
}) => {
  const [copied, setCopied] = useState(false);
  const compatible = isFFCompatible(item.preview);
      ...
      <button
        ...
        className={clsx(
          ...
          copied
            ...
            : "bg-white/[0.06] border border-white/[0.08] text-gray-400 hover:bg-amber-500/15 hover:text-amber-300 hover:border-amber-500/30"
        )}
      >
        {copied ? (
          <>
...
export const FreeFireNickGenerator = () => {
  ...
  const styles = useMemo(
    () => generateFreeFireNickStyles(effectiveInput),
    [effectiveInput]
  );
  ...
  const copyInvisible = async () => {
    try {
      await navigator.clipboard.writeText(FF_INVISIBLE_CHAR);
  ...
  const charColor = clsx({
    "text-gray-500": charCount <= 8,
    "text-amber-400 font-bold": charCount >= 9 && charCount <= 12,
    "text-rose-500 font-black": charCount >= 13,
  });
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-[#0B0F1A] pt-10 pb-10 sm:pt-16 sm:pb-14 border-b border-white/[0.03]">
        {/* Gradient backdrop */}
        <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-600/10 via-[#0B0F1A] to-[#0B0F1A]" />
        <div className="absolute inset-0 z-0 bg-[radial-gradient(ellipse_at_bottom_right,_var(--tw-gradient-stops))] from-orange-600/5 via-transparent to-transparent" />
        <FloatingParticles />

        <Container className="relative z-10 flex flex-col items-center text-center">
          {/* Badge */}
          <span className="mb-5 inline-flex items-center gap-1.5 rounded-full border border-amber-400/20 bg-amber-500/10 px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-amber-300">
            <Flame className="w-3 h-3" /> Free Fire Uyumlu Araç
          </span>
          ...
          <p className="mt-4 max-w-lg text-sm sm:text-base text-gray-500 font-medium px-6">
            Free Fire için şekilli nick yazma aracı. Nickinizi yazın, anında havalı
            nicklerinizi görün. Tek tıkla kopyalayın.
          </p>
          ...
          <div className="mt-8 sm:mt-12 w-full max-w-2xl px-4 sm:px-0">
            <div className="relative group">
              {/* Glow ring */}
              <div className="absolute -inset-1 rounded-3xl bg-gradient-to-r from-amber-500 to-orange-500 opacity-0 blur-lg transition-opacity duration-500 group-focus-within:opacity-20" />

              <div className="relative rounded-2xl bg-[#0F172A] border border-white/[0.08] p-2 shadow-2xl shadow-black/50">
                <div className="flex flex-col rounded-xl bg-[#0B0F1A] transition-all duration-300 focus-within:ring-2 focus-within:ring-amber-500/30">
                  {/* Input header */}
                  <div className="flex items-center justify-between px-4 pt-3 pb-0">
                    <label
                      htmlFor="ff-nick-input"
                  ...
                  <input
                    ...
                    maxLength={30}
                    className="w-full bg-transparent px-4 py-4 text-xl sm:text-2xl md:text-3xl font-bold text-white placeholder:text-gray-700 focus:outline-none caret-amber-400"
                  />
                </div>
              </div>
            </div>
            ...
            {charCount > FF_NICK_LIMIT && (
              <div className="mt-2 flex items-center gap-2 text-rose-400 text-xs font-bold px-2">
                ...
                {FF_NICK_LIMIT} karakterden uzun nickler Free Fire&apos;da
                kullanılamaz.
              </div>
            )}
          </div>
          ...
          <div className="mt-6 w-full max-w-2xl px-4 sm:px-0">
            <button
              ...
              className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.15em] text-gray-500 hover:text-amber-300 transition-colors"
            >
            ...
            >
              <div className="flex flex-wrap gap-1.5 sm:gap-2">
                {FF_SYMBOLS.map((sym, i) => (
                  <button
                    key={i}
                    type="button"
                    onClick={() => insertSymbol(sym)}
                    className="rounded-lg bg-[#1F2937] border border-white/[0.06] w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center text-base sm:text-lg text-gray-300 font-medium transition-all hover:bg-amber-500/15 hover:border-amber-500/30 hover:text-amber-300 hover:scale-110 active:scale-90"
                  >
                    {sym}
                  </button>
                ))}
          ...
          <div className="mx-auto max-w-3xl px-2 sm:px-0">
            {!mounted ? (
              ...
              <div className="flex flex-col gap-10 relative z-10 w-full">
                ...
                {isDefault && (
                  <div className="rounded-2xl bg-gradient-to-r from-amber-500/[0.08] to-orange-500/[0.04] p-5 border border-amber-400/15">
                ...
                <section className="space-y-5">
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.15em] text-amber-400/90">
                  ...
                  <section className="space-y-5">
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.15em] text-orange-400/90">
                        <Flame className="w-4 h-4" /> Gamer Nickleri
                      </div>
                ...
                {/* Note */}
                <div className="rounded-2xl bg-gradient-to-r from-amber-500/[0.06] to-transparent p-5 border border-amber-400/10">
                  <p className="text-xs font-medium text-amber-300/80">
                    <span className="font-bold text-amber-200">Not:</span>{" "}
                    Free Fire&apos;da nick en fazla {FF_NICK_LIMIT} karakter
          ...
          <div className="mx-auto max-w-3xl">
            <div className="flex items-center gap-4 mb-8">
              <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.15em] text-purple-400/80">
                <Sparkles className="w-4 h-4" /> Hazır Free Fire Nickleri
              </div>
              <div className="h-px flex-1 bg-gradient-to-r from-purple-500/20 to-transparent" />
            </div>

            <div className="grid gap-2.5 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {FF_READY_NICKS.map((nick, i) => (
      ...
      {toast && (
        <div className="pointer-events-none fixed bottom-6 left-0 right-0 z-[100] flex justify-center px-4">
          <span className="rounded-full bg-gradient-to-r from-amber-600 to-orange-600 px-6 py-3 text-sm font-black text-white shadow-2xl shadow-amber-900/50">
            {toast}
          </span>
        </div>
      )}
